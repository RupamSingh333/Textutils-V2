...
function App() {
  ...
  const togglemode = () => {
    ...
    setMode(newMode);
    document.body.style.backgroundColor = newMode === "dark" ? "rgb(33 37 41)" : "white";
    toast.success(`${newMode.charAt(0).toUpperCase() + newMode.slice(1)} mode has been enabled`); // Display success toast
    document.title = `TextUtils ${newMode.charAt(0).toUpperCase() + newMode.slice(1)} Mode`; // Change document title based on mode
